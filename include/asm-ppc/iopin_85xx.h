...
static __inline__ void iopin_set_high (iopin_t * iopin)
{
	volatile uint *datp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.pdata;
	datp[iopin->port * 8] |= (1 << (31 - iopin->pin));
}

static __inline__ void iopin_set_low (iopin_t * iopin)
{
	volatile uint *datp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.pdata;
	datp[iopin->port * 8] &= ~(1 << (31 - iopin->pin));
}

static __inline__ uint iopin_is_high (iopin_t * iopin)
{
	volatile uint *datp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.pdata;
...
static __inline__ void iopin_set_in (iopin_t * iopin)
{
	volatile uint *dirp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.pdira;
	dirp[iopin->port * 8] &= ~(1 << (31 - iopin->pin));
}

static __inline__ uint iopin_is_out (iopin_t * iopin)
{
	volatile uint *dirp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.pdira;
...
static __inline__ void iopin_set_act (iopin_t * iopin)
{
	volatile uint *odrp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.podra;
	odrp[iopin->port * 8] &= ~(1 << (31 - iopin->pin));
}

static __inline__ uint iopin_is_odr (iopin_t * iopin)
{
	volatile uint *odrp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.podra;
	return (odrp[iopin->port * 8] >> (31 - iopin->pin)) & 1;
}

static __inline__ uint iopin_is_act (iopin_t * iopin)
{
	volatile uint *odrp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.podra;
...
static __inline__ void iopin_set_gen (iopin_t * iopin)
{
	volatile uint *parp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.ppara;
	parp[iopin->port * 8] &= ~(1 << (31 - iopin->pin));
}

static __inline__ uint iopin_is_ded (iopin_t * iopin)
{
	volatile uint *parp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.ppara;
	return (parp[iopin->port * 8] >> (31 - iopin->pin)) & 1;
}

static __inline__ uint iopin_is_gen (iopin_t * iopin)
{
	volatile uint *parp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.ppara;
	return ((parp[iopin->port * 8] >> (31 - iopin->pin)) & 1) ^ 1;
}

static __inline__ void iopin_set_opt2 (iopin_t * iopin)
{
	volatile uint *sorp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.psora;
	sorp[iopin->port * 8] |= (1 << (31 - iopin->pin));
}

static __inline__ void iopin_set_opt1 (iopin_t * iopin)
{
	volatile uint *sorp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.psora;
	sorp[iopin->port * 8] &= ~(1 << (31 - iopin->pin));
}

static __inline__ uint iopin_is_opt2 (iopin_t * iopin)
{
	volatile uint *sorp = &((immap_t *) CFG_IMMR)->im_cpm.im_cpm_iop.psora;
